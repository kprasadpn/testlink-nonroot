...
$app.helpers.setupDatabase = function(databaseHandler) {
  ...
  databaseHandler.database(databaseHandler.connection.name).executeFile(schemaFi
le);
  const app = $app;
  $file.substitute(
    dataSqlFile,
...
',
    `'${app.username}',MD5('${app.password}'), 8,'${app.email}', '${app.username
}','Administrator', '${app.language}'`,
    {abortOnUnmatch: true}
  );
  $file.substitute(
    dataSqlFile,
  ...
  databaseHandler.database(databaseHandler.connection.name).executeFile(dataSqlF
ile);
  databaseHandler.database(databaseHandler.connection.name).executeFile(function
SqlFile);
  databaseHandler.database(databaseHandler.connection.name).set('users', 'locale
', $app.language, {where: {id: '1'}});
};

$app.helpers.getDBVersion = function(databaseHandler) {
  return databaseHandler.database(databaseHandler.connection.name).get('db_versi
on', 'version')
    .toString()
    .replace(/^DB\s+/, '');
};
...
');
  ...
  sortedApplicableDirs.forEach((d) => {
    sqlFiles = sqlFiles.concat($file.glob($file.join(baseMigrationDir, d, 'mysql
', `DB.${d}`, '*', '*.sql'),
      {exclude: ['**/.*']}));
  });
  return sqlFiles;
...
$app.helpers.upgradeDatabase = function(databaseHandler) {
  ...
  if (semver.gt(appVersion, dbVersion)) {
    ...
    if (appVersion.major === dbVersion.major && appVersion.minor === dbVersion.m
inor) {
      ...
      sqlFiles.forEach((sqlFile) => {
        ...
        databaseHandler.database(databaseHandler.connection.name).execute($file.
read(sqlFile));
      });
    } else {
      $app.info('Major versions upgrade detected. Please upgrade the application
 manually');
    }
  }
};
