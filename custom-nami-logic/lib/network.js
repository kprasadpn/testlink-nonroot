...
function networkFunctions(opts) {
   ...
   * @param {string} [options.url='myip.bitnami.com'] - Server's url to obtain the ip from.
  ...
  function getPublicIp(options) {
    options = _.defaults(options || {}, {host: 'myip.bitnami.com', port: 80});
    let foundIp = '';
 ...
 * @example
 * getURL('example.com', {protocol: 'https', auth: 'user:bitnami'})
 * // => 'https://user:bitnami@bitnami.com'
 * @example
